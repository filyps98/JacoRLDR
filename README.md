...
This controller is used to drive the robot's end-effector position and orientation. It allows to control robots with redundant DOF by avoiding joint limits and minimizing joint torques.
...
We can then express these actions as individual reward functions that need to be maximized by the policy.
...
SAC is composed of two different types of networks:
* The Policy Network is the function that models the system. It uses VGG-16 first layers to extract relevant features from the images to increase the speed of training. The input is composed of an RGB image and a State Vector, while the output is a Gaussian distribution.

<p align="center">
  <img width="511" alt="image" src="https://github.com/filyps98/JacoRLDR/assets/21342982/6064fa66-309c-48ce-914c-18e5b2bdf6fd">
</p>

* The Soft-Q Value Network is the function that evaluates the current policy. The input is made up of the RGB image, the State Vector, and the Action Vector, it also uses VGG-16 to pre-process the image. The output is one-dimensional and it indicates how good is an action for a specific state
...
In order to set up the program, follow the instructions contained in the [mujoco-py](https://github.com/openai/mujoco-py) README, and install [mujoco 2.1.0](https://github.com/google-deepmind/mujoco/releases/tag/2.1.0). 
...
python train.py
```
While for testing the model, type:
```python
python test.py
```
