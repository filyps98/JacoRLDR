...
#If a model exists already use it
if (os.path.exists(model_path+"\_q1") and os.path.exists(model_path+"\_q2")):
    sac_trainer.load_model(model_path)
    
#This is all for using model, the text file has all the dataset the model was trained on, I find the datasets that are not in the model and I continue training the model and save the dataset in the file
...
#while(replay_buffer.load_next_dataset(path_summary)):
    ...
    #for i in range(1, math.ceil(replay_buffer_size/batch_size)):
        #for i in range(initial_update_itr):
            #=sac_trainer.update(batch_size, reward_scale=10., auto_entropy=AUTO_ENTROPY, target_entropy=-0.1*action_dim, train_policy = False)
    ...
    for step in range(max_steps):
        ...
        if len(replay_buffer) > batch_size:
            for i in range(update_itr):
                _=sac_trainer.update(batch_size, reward_scale=10., auto_entropy=AUTO_ENTROPY, target_entropy=-0.7*action_dim, train_policy = True)
